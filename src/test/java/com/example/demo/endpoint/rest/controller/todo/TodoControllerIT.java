...
class TodoControllerIT extends FacadeIT {
    ...
    void createTodo_should_create_and_return_201() {
        ...
        ResponseEntity<TodoResponse> response = restTemplate.postForEntity("/todos", request, TodoResponse.class);
    ...
    void getTodoById_should_return_the_created_todo() {
        ...
        TodoResponse created = restTemplate.postForEntity("/todos", request, TodoResponse.class).getBody();
    ...
    void getTodoById_nonExistent_shouldReturn404() {
        ResponseEntity<Object> response = restTemplate.getForEntity("/todos/id-inexistant", Object.class);
    ...
    void updateTodo_should_modify_the_todo() {
        var createRequest = CreateTodoRequest.builder().title("Avant modif").description("desc").build();
        TodoResponse created = restTemplate.postForEntity("/todos", createRequest, TodoResponse.class).getBody();
        ...
        ResponseEntity<TodoResponse> response =
                restTemplate.exchange(
                        org.springframework.http.RequestEntity.put(java.net.URI.create("/todos")).body(updateRequest),
                        TodoResponse.class);
    ...
    void getTodos_should_list_todos_and_filter_by_isCompleted() {
        ...
        TodoResponse done = restTemplate.postForEntity("/todos", doneRequest, TodoResponse.class).getBody();
        ...
        ResponseEntity<List<TodoResponse>> allTodos =
                restTemplate.exchange(
                        "/todos", HttpMethod.GET, null, new ParameterizedTypeReference<List<TodoResponse>>() {});
